package com.sym.dataStructure.queue.ArrayQueue;
 ...
 *
 */
public class CircleArrayQueueV2 implements ISymQueue {
    ...
    public Object peek() {
        if( front == rear && concurrentSize == 0 ){
            throw new RuntimeException("队列为空~无法读取队首元素");
        }
    ...
    public void offer(Object o){
        //首先判断队列是否满了
        if( isFull() ) throw new RuntimeException("队列已满,无法添加新元素");
        // 将新元素放在队尾指标rear指向的位置
        elements[rear] = o;
    ...
    public Object poll() {
        // 首先判断队列是否为空
        if( isEmpty() ) throw new RuntimeException("队列为空,无法取出队首元素");
        // 取出队首指标指向的数组位置的元素
